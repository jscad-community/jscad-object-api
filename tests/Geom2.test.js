...
import { geom2, mat4 } from '@jscad/modeling'

import { Geom2 } from '../src/index.js'
...
test('Geom2 (clone color reverse)', (t) => {
  ...
  t.is(geom1.geometry.outlines.length, 1)

  let geom2 = geom1.clone()

  t.not(geom1, geom2)

  t.is(geom2.geometry.outlines.length, 1)
...
test('Geom2 (transform functions)', (t) => {
  ...
  sides = geom2.toSides()
  t.is(sides.length, 3)
  t.deepEqual(sides[0], [[0, 0], [0, 5]])
  t.deepEqual(sides[1], [[0, 5], [-5, 5]])
  t.deepEqual(sides[2], [[-5, 5], [0, 0]])
  ...
  sides = geom2.toSides()
  t.is(sides.length, 3)
  t.deepEqual(sides[0], [[0, 0], [10, 0]])
  t.deepEqual(sides[1], [[10, 0], [10, 2.5]])
  t.deepEqual(sides[2], [[10, 2.5], [0, 0]])
  ...
  sides = geom2.toSides()
  t.is(sides.length, 3)
  t.deepEqual(sides[0], [[0, 0], [5, 0]])
  t.deepEqual(sides[1], [[5, 0], [5, 5]])
  t.deepEqual(sides[2], [[5, 5], [0, 0]])
  ...
  sides = geom2.toSides()
  t.is(sides.length, 3)
  t.deepEqual(sides[0], [[-5, 5], [0, 5]])
  t.deepEqual(sides[1], [[0, 5], [0, 10]])
  t.deepEqual(sides[2], [[0, 10], [-5, 5]])
  ...
  t.is(sides.length, 3)
  t.deepEqual(sides[0], [[0, 0], [10, 0]])
  t.deepEqual(sides[1], [[10, 0], [10, 10]])
  t.deepEqual(sides[2], [[10, 10], [0, 0]])
})
